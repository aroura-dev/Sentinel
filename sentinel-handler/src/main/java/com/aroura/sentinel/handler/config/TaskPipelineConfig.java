 ...
 * 责任链顺序：
 *   0. DiscardAction（复用 sentinel）
 *   1. ShieldAction（复用 sentinel）
 *   2. DeduplicationAction（复用 sentinel）
 *   3. SensWordsAction（复用 sentinel）
 *   4. AgentContentAction（新增：通知文案生成）
 *   5. SendMessageAction（复用 sentinel：渠道发送）
 *
 * @author Sentinel
 */
@Configuration
