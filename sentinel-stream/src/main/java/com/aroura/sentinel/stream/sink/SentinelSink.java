package com.aroura.sentinel.stream.sink;
...
import com.aroura.sentinel.stream.utils.LettuceRedisUtils;
 ...
 */
@Slf4j
public class SentinelSink implements SinkFunction<AnchorInfo> {
    ...
    private void realTimeData(AnchorInfo info) {
        try {
            LettuceRedisUtils.pipeline(redisAsyncCommands -> {
                 ...
                 * key:Sentinel:MessageId:{messageId},listValue:[{timestamp,state,businessId},{timestamp,state,businessId}]
                 */
                String redisMessageKey = CharSequenceUtil.join(StrPool.COLON, SentinelConstant.CACHE_KEY_PREFIX, SentinelConstant.MESSAGE_ID, info.getMessageId());
            ...
            log.error("SentinelSink#invoke error: {}", Throwables.getStackTraceAsString(e));
        }
    }
}
