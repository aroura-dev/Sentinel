  ...
  <el-drawer v-model="detailVisible" :title="detail ? '用户详情 · ' + detail.username : '用户详情'" size="440px" destroy-on-close>
    <template v-if="detail">
      <el-descriptions :column="1" border size="small">
        ...
        <el-descriptions-item label="状态"><el-tag :type="detail.status === '1' ? 'success' : 'danger'" size="small">{{ detail.status === '1' ? '启用' : '停用' }}</el-tag></el-descriptions-item>
      ...
      <div class="ud-actions">
        ...
        <el-button v-if="!isSelf(detail)" :type="detail.status === '1' ? 'danger' : 'success'" plain @click="toggleFromDetail">{{ detail.status === '1' ? '停用账号' : '启用账号' }}</el-button>
...
const batchDisableable = computed(() => selected.value.filter((r) => !(r.username === me && r.status === '1')))
...
async function batchToggle(target) {
  ...
  try {
    for (const r of rows) {
      ...
      if ((target === '0' && r.status === '1') || (target === '1' && r.status !== '1')) await userToggle(r.id)
    }
