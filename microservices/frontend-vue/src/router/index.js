...
import { MENU, rolesForPath, pathAllowed, landingFor } from '../utils/menu'
import { readKey, readPerms } from '../utils/authStorage'
...
router.beforeEach((to, from, next) => {
  const token = readKey('sentinel_token')
  const home = landingFor(readKey('sentinel_role'))
  ...
  // 该角色的落地首页恒放行：作为登录/越权后的兜底落地页，避免“跳回自身”的无限循环
  if (to.path === home) {
    next()
    return
  }
  ...
  if (perms.length) {
    ...
    const role = readKey('sentinel_role')
