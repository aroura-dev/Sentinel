...
// 登录成功/越权后的落地首页：按角色选择，避免把 MERCHANT/客服送到 ADMIN 专属的运营总览
export function landingFor(role = '') {
  const r = String(role || '').toUpperCase()
  if (r === ROLES.MERCHANT) return '/tms/seller-dashboard'
  if (r === ROLES.CUSTOMER_SERVICE) return '/workbench'
  if ([ROLES.ADMIN, ROLES.OPERATOR, ROLES.FINANCE].includes(r)) return '/dashboard'
  return '/workbench' // 未知/空角色 → 全角色可见的今日待办
}
