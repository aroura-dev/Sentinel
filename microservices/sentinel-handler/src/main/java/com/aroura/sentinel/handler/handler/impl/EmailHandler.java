...
import com.aroura.sentinel.support.utils.SentinelFileUtils;
...
public class EmailHandler extends BaseHandler{
    ...
    public boolean handler(TaskInfo taskInfo) {
        ...
        if (account == null) {
            log.warn("EmailHandler 账号未配置，跳过实际发送 bizId={}", taskInfo.getBizId());
            return false;
        }
        try {
            List<File> files = CharSequenceUtil.isNotBlank(emailContentModel.getUrl()) ? SentinelFileUtils.getRemoteUrl2File(dataPath, CharSequenceUtil.split(emailContentModel.getUrl(), StrPool.COMMA)) : null;
    ...
    private MailAccount getAccountConfig(Integer sendAccount) {
        ...
        if (account == null || CharSequenceUtil.isBlank(account.getHost())) {
            log.warn("EmailHandler#getAccount 渠道账号未配置或无效 sendAccount={}", sendAccount);
            return null;
        }
        try {
