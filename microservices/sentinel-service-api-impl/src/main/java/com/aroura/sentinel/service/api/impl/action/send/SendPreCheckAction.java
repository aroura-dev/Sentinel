...
public class SendPreCheckAction implements BusinessProcess<SendTaskModel> {
    ...
    public void process(ProcessContext<SendTaskModel> context) {
        ...
        List<MessageParam> resultMessageParamList = messageParamList.stream()
                .filter(messageParam -> messageParam != null && !CharSequenceUtil.isBlank(messageParam.getReceiver()))
        ...
        if (resultMessageParamList.stream().anyMatch(messageParam -> messageParam.getReceiver().split(StrPool.COMMA).length > SentinelConstant.BATCH_RECEIVER_SIZE)) {
