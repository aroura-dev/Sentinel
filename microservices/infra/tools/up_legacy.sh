...
# 前提：austin-mysql(:3307, 源 sentinel 基建) 在跑；与 auth 共享 Redis(:6381) 使 token 互通。
# 用法： bash infra/tools/up_legacy.sh
# ============================================================
set -e
SRC=/d/internship/sentinel
JAR="$SRC/austin-web/target/austin-web-0.0.1-SNAPSHOT.jar"
...
if [ ! -f "$JAR" ]; then
  ...
  (cd "$SRC" && mvn -s "$SETTINGS" -pl austin-web -am package -DskipTests)
fi

echo "启动 legacy :8090 (DB=austin@3307, Redis=6381 共享 token)"
(cd "$SRC" && DASHSCOPE_API_KEY=sk-dummy java -jar "$JAR" \
  --server.port=8090 \
  --austin.database.ip=127.0.0.1 --austin.database.port=3307 \
  --austin.redis.ip=127.0.0.1 --austin.redis.port=6381 --austin.redis.password=austin \
  > /tmp/mono.log 2>&1 &)
