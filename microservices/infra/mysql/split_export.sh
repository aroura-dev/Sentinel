#!/usr/bin/env bash
# ============================================================
# sentinel-ms 分库导出：从 monolith 的 austin 库(3307) 按服务归属
# 把每张表 structure+data 导出成各服务 initdb 的 00_schema_data.sql，
# 保证与新微服务各进程的独立 MySQL 数据完全一致（快照式、幂等可重建）。
# 前提：austin-mysql(127.0.0.1:3307, root/root123_A) 已灌好全量种子。
...
for svc in "${svcs[@]}"; do
  ...
  mkdir -p "$(dirname "$out")"
  {
    echo "-- sentinel-ms : austin_${svc}（由 模块化单体版 库快照导出生成，勿手改）"
    echo "CREATE DATABASE IF NOT EXISTS austin_${svc} DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;"
    echo "USE austin_${svc};"
    echo "SET NAMES utf8mb4;"
    echo ""
  } > "$out"
  # mysqldump 在 austin-mysql 容器内执行（按 socket 连本容器 MySQL）
  # shellcheck disable=SC2086
  docker exec austin-mysql mysqldump -uroot -proot123_A --default-character-set=utf8mb4 --skip-comments --no-tablespaces austin ${!svc} >> "$out"
