...
The stack publishes only the gateway (`8080`) and the demo console (`5175`). The business services and their databases stay on the internal compose network.

This is a security property, not tidiness. Downstream services trust the `X-User-Name` / `X-User-Role` headers unconditionally, so publishing `8081`–`8084` would let anyone who can reach the host forge those headers and bypass the gateway entirely. `agent-service`'s `/internal/**` and `msg-service`'s `/send` are not even under `/api`, so they sit outside the role interceptor's scope.

To reach the debug ports from the host (a local MySQL client, calling `8081`–`8084` directly), opt in:

```bash
DEV_PORTS=1 bash infra/tools/up.sh
```

The `compose.dev.yml` override adds them back bound to `127.0.0.1` only — other machines on the LAN still cannot reach them.
